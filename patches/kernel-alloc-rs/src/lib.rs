...
use nt::{
    MmAllocateContiguousMemorySpecifyCacheNode, MmFreeContiguousMemory, PoolFlags, MM_ANY_NODE_OK,
};
...
unsafe impl GlobalAlloc for KernelAlloc {
    unsafe fn alloc(&self, layout: Layout) -> *mut u8 {
        // #[cfg(feature = "pool-tag")]
        // let pool = nt::ExAllocatePoolWithTag(PoolType::NonPagedPool, layout.size(),
        // POOL_TAG);

        // #[cfg(not(feature = "pool-tag"))]
        // let pool = nt::ExAllocatePool(PoolType::NonPagedPool, layout.size());

        let pool = nt::ExAllocatePool2(PoolFlags::NonPaged, layout.size(), POOL_TAG);
        if pool.is_null() {
