...
use crate::clock;
use crate::handshake::{LABEL_COOKIE, LABEL_MAC1};
use crate::seconds;
use crate::{HandshakeInit, HandshakeResponse, Packet, Tunn, TunnResult, WireGuardError};
...
const RESET_PERIOD: Duration = seconds(1);
...
impl RateLimiter {
    pub fn new(public_key: &crate::x25519::PublicKey, limit: u64) -> Self {
        ...
        RateLimiter {
            ...
            last_reset: Mutex::new(clock::now()),
        }
    }
    ...
    pub fn reset_count(&self) {
        ...
        if current_time - *last_reset_time >= RESET_PERIOD {
    ...
    fn current_cookie(&self, addr: IpAddress) -> Cookie {
        let mut addr_bytes = [0u8; 16];

        match addr {
            IpAddress::Ipv4(a) => addr_bytes[..4].copy_from_slice(&a.as_bytes()),
            IpAddress::Ipv6(a) => addr_bytes[..].copy_from_slice(&a.as_bytes()),
        }
        ...
        let cur_counter = ((clock::now() - self.start_time).to_seconds() as u64) / COOKIE_REFRESH;
