...
use crate::errors::WireGuardError;
...
// A const implementation for creating a Duration with "small" seconds
const fn seconds(sec: u64) -> Duration {
    // HACK: accesses the private fields of hifitime::Duration
    // requires modification of source file of hifitime
    Duration {
        centuries: 0,
...
pub enum TunnResult<'a> {
    ...
    WriteToTunnelV4(&'a mut [u8], Ipv4Address),
    WriteToTunnelV6(&'a mut [u8], Ipv6Address),
}
...
/// S point-to-point WireGuard connection
#[derive(Debug)]
pub struct Tunn {
...
impl Tunn {
    ...
    pub fn parse_incoming_packet(src: &[u8]) -> Result<Packet, WireGuardError> {
        ...
        Ok(match (packet_type, src.len()) {
            ...
            (DATA, DATA_OVERHEAD_SZ..=core::usize::MAX) => Packet::PacketData(PacketData {
                receiver_idx: u32::from_le_bytes(src[4..8].try_into().unwrap()),
    ...
    pub fn dst_address(packet: &[u8]) -> Option<IpAddress> {
        if packet.is_empty() {
            return None;
        }
        ...
        match packet[0] >> 4 {
            4 if packet.len() >= IPV4_MIN_HEADER_SIZE => {
                let addr_bytes: [u8; IPV4_IP_SZ] = packet
                    ...
                    .try_into()
                    .unwrap();
                Some(IpAddress::Ipv4(Ipv4Address::from_bytes(&addr_bytes)))
            }
            6 if packet.len() >= IPV6_MIN_HEADER_SIZE => {
                let addr_bytes: [u8; IPV6_IP_SZ] = packet
                    ...
                    .try_into()
                    .unwrap();
                Some(IpAddress::Ipv6(Ipv6Address::from_bytes(&addr_bytes)))
            }
            _ => None,
        }
    }
    ...
    fn validate_decapsulated_packet<'a>(&mut self, packet: &'a mut [u8]) -> TunnResult<'a> {
        let (computed_len, src_ip_address) = match packet.len() {
            ...
            _ if packet[0] >> 4 == 4 && packet.len() >= IPV4_MIN_HEADER_SIZE => {
                ...
                (
                    ...
                    IpAddress::Ipv4(Ipv4Address::from_bytes(&addr_bytes)),
                )
            }
            _ if packet[0] >> 4 == 6 && packet.len() >= IPV6_MIN_HEADER_SIZE => {
                ...
                (
                    ...
                    IpAddress::Ipv6(Ipv6Address::from_bytes(&addr_bytes)),
                )
            }
        ...
        match src_ip_address {
            IpAddress::Ipv4(addr) => TunnResult::WriteToTunnelV4(&mut packet[..computed_len], addr),
            IpAddress::Ipv6(addr) => TunnResult::WriteToTunnelV6(&mut packet[..computed_len], addr),
        }
    }
...
mod tests {
    ...
    #[cfg(feature = "mock-instant")]
    use crate::timers::{REKEY_AFTER_TIME, REKEY_TIMEOUT};
    ...
    fn new_handshake_after_two_mins() {
        ...
        // after the timeout
        mock_instant::MockClock::advance_std(Duration::from_secs(1));
        assert!(matches!(their_tun.update_timers(&mut []), TunnResult::Done));
        assert!(matches!(
