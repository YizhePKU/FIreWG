...
use hifitime::{Duration, Epoch, TimeUnits, Unit, UNIX_REF_EPOCH};
use rand_core::OsRng;
...
impl TimeStamper {
    ...
    pub fn new() -> TimeStamper {
        TimeStamper {
            duration_at_start: clock::now() - UNIX_REF_EPOCH,
            instant_at_start: clock::now(),
        }
    }
    ...
    pub fn stamp(&self) -> [u8; 12] {
        ...
        let stamp = clock::now() - self.instant_at_start + self.duration_at_start;
        let sec = stamp.floor(1f64.seconds()).to_seconds() as u64;
        let subsec_nanos = (stamp - stamp.floor(1f64.seconds())).to_unit(Unit::Nanosecond) as u32;
        ext_stamp[0..8].copy_from_slice(&(sec + TAI64_BASE).to_be_bytes());
        ext_stamp[8..12].copy_from_slice(&subsec_nanos.to_be_bytes());
        ext_stamp
    }
}
...
impl Tai64N {
    ...
    fn parse(buf: &[u8; 12]) -> Result<Tai64N, WireGuardError> {
        ...
        let (sec_bytes, nano_bytes) = buf.split_at(core::mem::size_of::<u64>());
...
impl Handshake {
    ...
    pub(crate) fn timer(&self) -> Option<Epoch> {
        match self.state {
    ...
    pub(super) fn receive_handshake_response(
        ...
        let rtt_time = clock::now() - state.time_sent;
        self.last_rtt = Some(rtt_time.to_unit(Unit::Millisecond) as u32);
