...
pub(crate) const REKEY_AFTER_TIME: Duration = seconds(12);
const REJECT_AFTER_TIME: Duration = seconds(180);
const REKEY_ATTEMPT_TIME: Duration = seconds(90);
pub(crate) const REKEY_TIMEOUT: Duration = seconds(5);
const KEEPALIVE_TIMEOUT: Duration = seconds(10);
const COOKIE_EXPIRATION_TIME: Duration = seconds(120);
...
impl Timers {
    pub(super) fn new(persistent_keepalive: Option<Duration>, reset_rr: bool) -> Timers {
        Timers {
            is_initiator: false,
            ...
            persistent_keepalive: persistent_keepalive.unwrap_or(seconds(0)),
    ...
    pub(super) fn clear(&mut self) {
        let now = clock::now() - self.time_started;
...
impl Tunn {
    ...
    pub(super) fn timer_tick_session_established(
        ...
        self.timers.session_timers[session_idx % crate::N_SESSIONS] = self.timers[TimeCurrent];
    ...
    pub fn update_timers<'a>(&mut self, dst: &'a mut [u8]) -> TunnResult<'a> {
        ...
        let now = time - self.timers.time_started;
...

        {
            ...
            if let Some(time_init_sent) = self.handshake.timer() {
                ...
                if clock::now() - time_init_sent >= REKEY_TIMEOUT {
                ...
                if !handshake_initiation_required {
                    ...
                    if persistent_keepalive > Duration::ZERO
    ...
    pub fn time_since_last_handshake(&self) -> Option<Duration> {
        ...
        if self.sessions[current_session % super::N_SESSIONS].is_some() {
            let duration_since_tun_start = clock::now() - self.timers.time_started;
